...
const SummaryCard = ({ title, value, percentage, icon, trend, className }: SummaryCardProps) => {
  return (
    <Card className={cn("transition-all hover:shadow-md", className)}>
      <CardContent className="p-6">
        <div className="flex justify-between items-start">
          ...
          <div className={cn(
            "p-3 rounded-full",
            trend === 'up' ? 'bg-green-100 text-profit' : 'bg-red-100 text-loss'
          )}>
            {icon}
          </div>
        </div>
        <div className="flex items-center mt-3 text-sm">
          <span className={cn(
            ...
            {trend === 'up' ? <TrendingUpIcon size={16} className="mr-1" /> : <TrendingDownIcon size={16} className="mr-1" />}
            {percentage}%
          </span>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 dashboard-section">
      ...
      <SummaryCard
        ...
        value={`$${spendings.toLocaleString()}`}
      ...
      <Card className={cn(
        ...
        <CardContent className="p-6">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Net Cash Status</p>
              <h3 className={cn(
                "text-2xl font-bold mt-1",
                isProfit ? "text-profit" : "text-loss"
              )}>
                {isProfit ? `+$${netCash.toLocaleString()}` : `-$${Math.abs(netCash).toLocaleString()}`}
              </h3>
            </div>
            <div className={cn(
              "p-3 rounded-full",
          ...
          <div className="flex items-center mt-3">
            ...
            <span className="text-sm text-muted-foreground ml-2">for current period</span>
          </div>
        </CardContent>
      </Card>
    </div>
